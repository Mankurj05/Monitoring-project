...
ORDERS_URL = os.getenv("DOWNSTREAM_ORDERS_URL", "http://orders:8002/orders")
...
@app.get("/checkout")
def checkout(request: Request, item_id: str, quantity: int = 1, fail_payments: bool = False):
    ...
    orders_response = requests.get(
        ORDERS_URL,
        params={"item_id": item_id, "quantity": quantity, "fail_payments": fail_payments},
        headers=headers,
        timeout=3,
    )
    ...
    payments_response = requests.get(
        PAYMENTS_URL,
        params={"amount": 25.5 * quantity, "fail": fail_payments},
        headers=headers,
        timeout=3,
    )
    if payments_response.status_code >= 500:
